        ...
        const fetchData = async () => {
            try {
                ...
                const servicesArray = Object.keys(userServices).map(key => ({
                    orderId: userServices[key].orderId || key, // Use saved orderId, fallback to key
                    userId: userId,
                            ...
                            {services.length > 0 ? (
                                services.map((service) => (
                                    <TableRow key={service.orderId}>
                                        <TableCell className="font-mono text-xs">{service.orderId.substring(service.orderId.length - 6)}</TableCell>
