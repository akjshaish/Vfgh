...
import { MoreHorizontal, Loader2, PlayCircle, PauseCircle, XCircle, Ban, ClipboardCopy } from 'lucide-react';
                ...
                for (const userId in usersData) {
                    ...
                    if (user.services) {
                        for (const serviceId in user.services) {
                            ...
                            allOrders.push({
                                ...service,
                                id: serviceId, // The key of the service object is its ID
                                userId: userId,
                                    ...
                                    <TableRow key={order.id}>
                                        <TableCell className="font-mono text-xs">
                                            <div className="flex items-center gap-1">
                                                <span>{order.id.substring(order.id.length - 6)}</span>
                                                <Button variant="ghost" size="icon" className="h-5 w-5" onClick={() => copyToClipboard(order.id)}>
                                        ...
                                        <TableCell className="text-right">
                                            ...
                                            <DropdownMenu>
                                                ...
                                                <DropdownMenuContent>
                                                    <DropdownMenuItem onClick={() => handleUpdateStatus(order.userId, order.id, 'Active')}>
                                                        <PlayCircle className="mr-2 h-4 w-4" /> Activate
                                                    </DropdownMenuItem>
                                                    <DropdownMenuItem onClick={() => handleUpdateStatus(order.userId, order.id, 'Suspended')}>
                                                        <PauseCircle className="mr-2 h-4 w-4" /> Suspend
                                                    </DropdownMenuItem>
                                                    <DropdownMenuItem onClick={() => handleUpdateStatus(order.userId, order.id, 'Terminated')}>
                                                        <XCircle className="mr-2 h-4 w-4" /> Terminate/Cancel
                                                    </DropdownMenuItem>
                                                    <DropdownMenuItem className="text-destructive" onClick={() => handleUpdateStatus(order.userId, order.id, 'Banned')}>
